...
const MotherMedicalTips = ({ data }) => {
  ...
  const getpredncystatus = (week) => {
    if (week < 12) {
      return `At ${week} weeks of gestation, a pregnancy is still in its early stages, marked by rapid development and crucial milestones for the growing fetus.`;
    } else if (week >= 12 && week < 28) {
      return `A pregnancy at ${week} weeks signifies the midpoint of the second trimester, a time when fetal development is rapidly advancing.`;
    } else {
      return `A pregnancy of ${week} weeks corresponds to the beginning of the third trimester, a critical stage in fetal development`;
    }
  };
  ...
  const handlePress = (item) => {
    navigation.navigate('MotherTips', { tips: item });
  };
      ...
      <View style={styles.container}>
        ...
        {clinicData.map((item, index) => (
          <View style={styles.card} key={index}>
            ...
            <Text style={styles.cardText}>
              <Text style={styles.cardHeader}>{getpredncystatus(item.week)} </Text>
            </Text>
...
export default MotherMedicalTips;
